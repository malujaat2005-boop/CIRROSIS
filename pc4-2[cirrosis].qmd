...
Para ilustrar el proceso de imputación múltiple de datos, utilizaremos el conjunto de datos `data_sm`. Este dataset incluye información de 418 pacientes adultos. Las variables registradas : **ID, Dias_Seguimiento , Estado , Medicamento , Edad, Sexo , Ascitis , Hepatomegalia , Aracnoides ,Edema, Bilirrubina ,Colesterol , Albumina , Cobre, Fosfatasa_Alcalina , SGOT , Trigliceridos, Plaquetas , Tiempo_Protrombina, Etapa** , entre otras. Algunos participantes presentan valores faltantes en al menos una de estas variables.
...
```{r}
data_sm |>
  select(ID , Dias_Seguimiento , Estado ,Medicamento , Sexo ,Ascitis , Hepatomegalia ,Aracnoides ,Colesterol) |>
  ggmice::plot_pattern(
    square = TRUE,
    rotate = TRUE)
```
...
```{r}
tabla_ID = data_sm |> 
  dplyr::select(ID , Dias_Seguimiento , Estado ,Medicamento , Sexo ,Ascitis , Hepatomegalia ,Aracnoides ,Colesterol) |>
...
```{r}
ggmice(data_imputada, aes(x = ID, y = Colesterol)) +
  geom_jitter(height = 0, width = 0.25) +
  geom_boxplot(width = 0.5, size = 1, alpha = 0.55, outlier.shape = NA) +
  labs(x = "Imputation number")
```

Para la variables Hepatomegalia

```{r}
ggmice(data_imputada, aes(x = ID, y = Hepatomegalia)) +
  geom_jitter(height = 0, width = 0.25) +
  geom_boxplot(width = 0.5, size = 1, alpha = 0.55, outlier.shape = NA) +
  labs(x = "Imputation number")
```
...
prop.table(table(data_imputada_l$Colesterol,
                 data_imputada_l$Hepatomegalia),
           margin = 2)
```
...
tabla_multi <-
  data_imputada |> 
  with(glm(ID  ~ Dias_Seguimiento  + Estado  +Sexo + Ascitis  + Hepatomegalia  + Aracnoides + Colesterol ,
...
```         
[1] "ID"               "Dias_Seguimiento" "Estado"           [4] "Medicamento"      "Sexo"             "Ascitis"          [7] "Hepatomegalia"    "Aracnoides"       "Colesterol"
```
